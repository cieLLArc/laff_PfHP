...
void LoopFour(int m, int n, int k, double *A, int ldA, double *B, int ldB, double *C, int ldC)
{
...
void LoopThree(int m, int n, int k, double *A, int ldA, double *Btilde, double *C, int ldC)
{
    ...
    double *Atilde = (double *)_mm_malloc(MC * KC * max_threads * sizeof(double), 64);

    int m_full_blocks = (m / (MC * max_threads)) * MC * max_threads;
    int remainder = m - m_full_blocks;
    int remainder_per_thread = ((remainder / max_threads) / MR + 1) * MR;

#pragma omp parallel for
    for (int i = 0; i < m_full_blocks; i += MC)
    {
        PackBlockA_MCxKC(MC, k, &alpha(i, 0), ldA, &Atilde[MC * KC * omp_get_thread_num()]);
        LoopTwo(MC, n, k, &Atilde[MC * KC * omp_get_thread_num()], Btilde, &gamma(i, 0), ldC);
    }

#pragma omp parallel for
    for (int i = m_full_blocks; i < m; i += remainder_per_thread)
    {
        int ib = min(remainder_per_thread, m - i);
